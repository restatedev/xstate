...
You can provide `{retry: true}` to execute the `fromPromise` actor with Restate's retry policy enabled; without this flag, any error will simply report back to the state machine, which matches native xstate behaviour.
